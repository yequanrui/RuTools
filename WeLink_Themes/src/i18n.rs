...
pub fn get(key: &str) -> &str {
    let res = match key {
        ...
        "community_theme" => ["Install the Official Community Theme. (Only theme switching is supported)", "安装官方社区版主题。（仅支持主题切换，不附带功能特性）"],
        "dev_theme" => ["Install the DevTheme. (More new features)", "安装DevTheme（主打新功能特性，支持版本WeLink7.x以上版本，WeLink升级后需要重装主题）"],
        "super_theme" => ["Install the SuperTheme. (More customized themes)", "安装SuperTheme（主打定制化主题，支持版本WeLink7.x以上版本，WeLink升级后需要重装主题）"],
        ...
        "default_style_path" => ["Default Extended Style Sheet", "主题默认扩展样式表路径"],
        "user_style_path" => ["User Extended Style Sheet", "用户扩展样式表路径"],
        "config_tips" => ["The configuration file", "配置文件"],
        "style_tips" => ["Extended style sheet ", "扩展样式表"],
        "skip_tips" => ["already exist. Skip the creation.", ""],
